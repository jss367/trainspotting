...
[Where in training it was seen](targets.md#where-in-training-it-was-seen).
...
Post-training sampling defaults to `--sample 1000 --seed 0`. Use the same
settings across the sampled layers so hits join to the labels and stored
context from that draw. Changing `--sample` or `--seed` changes the sampled
rows; refresh the other layers together to keep those joins consistent.

`search` and `grep` ask the same question of different amounts of data, and the
difference is the whole point of having both. `search` reads sampled rows, so
it hands back examples to read without providing an exact stage-wide count.
Even the default 1,000-row sample can miss a rare string. `grep` reads every row, so
...
An exact stage-wide count cannot come from a sample. `classify` and `ask`
default to 1,000 prompts per post-training stage; at 0.5% that is five expected
matches. Rarer patterns can still yield no matches, which does not establish
that the string is absent from the full mix.
...
The site searches the stored samples, whose sizes are recorded in each
artifact; post-training and corpus samples can have different sizes. This
finds examples rather than an exact count over the mix, where `grep` reads all
150,000 rows of this stage. A string in none of the samples can still occur
in thousands of rows in the full dataset.

Matching is literal, case-insensitive, and substring, the way ⌘F is. The page
does not download all stored samples to do it: `scripts/export_site_data.py`
...
- **Not a side.** It counts rows, not which half of the example the string is
  ...
  a side, but it searches a random sample. At a density of 100 per million
  rows, even the default 1,000-row draw has only about a 10% chance of finding
  any hit, so it will usually find none.
