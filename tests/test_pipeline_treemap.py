"""Run the site's pipeline-treemap assertions under pytest.

The treemap lives in docs/index.html, so its tests are JavaScript; this wrapper
...
SUITE = Path(__file__).resolve().parent / "site" / "pipeline_treemap.test.mjs"


@pytest.mark.skipif(shutil.which("node") is None, reason="node is not installed")
def test_pipeline_treemap_claims():
    proc = subprocess.run(
